...
class PistasModel extends \Com\Daw2\Core\BaseModel
{
    ...
    function mostrarReservasTenis($filtros,$tamPag)
    {
        $order = "desc";
        if (isset($filtros['order']) && $filtros['order'] == "fecha_asc") {
            $order = "asc";
        ...
        $procesado = $this->filtrosTenis($filtros);
        $condiciones = $procesado['condiciones'];
        $parametros = $procesado['parametros'];

        if (count($parametros) > 0) {
            $stmt = $this->pdo->prepare('SELECT * FROM reservaTenis INNER JOIN usuarios ON reservaTenis.id_usuario = usuarios.id WHERE ' . implode(' AND ', $condiciones) . " ORDER BY fecha_reserva " . $order." ".$limit);
            $stmt->execute($parametros);
            return $stmt->fetchAll();
        } else {
            $stmt = $this->pdo->prepare('SELECT * FROM reservaTenis INNER JOIN usuarios ON reservaTenis.id_usuario = usuarios.id ORDER BY fecha_reserva ' . $order." ".$limit);
            $stmt->execute($parametros);
            $datos = $stmt->fetchAll();
            foreach ($datos as $num => $d) {
                unset($datos[$num]['contraseña']);
    ...
    function countTenis($filtros) {
        ...
        if(count($parametros) > 0){
            $sql = 'SELECT * FROM reservaTenis INNER JOIN usuarios ON reservaTenis.id_usuario = usuarios.id WHERE '.implode(" AND ", $condiciones);
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute($parametros);
            $res = $stmt->fetchAll();
            return count($res);
            ...
            $stmt = $this->pdo->query('SELECT * FROM reservaTenis INNER JOIN usuarios ON reservaTenis.id_usuario = usuarios.id');
            $res = $stmt->fetchAll();
            return count($res);
        }
    }
